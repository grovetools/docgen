...
func (w *AstroWriter) WriteDoc(pkg, filename string, content []byte, meta DocMetadata) error {
	...
	if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil { //nolint:gosec // internal doc tool, predictable paths
		return fmt.Errorf("failed to create directory: %w", err)
	}
	return os.WriteFile(path, content, 0644) //nolint:gosec // internal doc tool output
}
...
func (w *AstroWriter) WriteAsset(pkg, assetType, filename string, data []byte) error {
	...
	return os.WriteFile(path, data, 0644) //nolint:gosec // internal doc tool output
}
...
func (w *AstroWriter) WriteManifest(manifest []byte) error {
	...
	return os.WriteFile(path, manifest, 0644) //nolint:gosec // internal doc tool output
}
