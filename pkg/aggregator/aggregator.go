...
func (a *Aggregator) aggregateConcepts(wsPath string, wsName string, docCfg *docgenConfig.DocgenConfig, distDest string, mode string, transform string) error {
	...
	conceptsDir := filepath.Join(filepath.Dir(docgenDir), "concepts")
	if !dirExists(conceptsDir) {
		return nil
	}
	...
	for _, entry := range entries {
		...
		cm, err := concept.LoadManifest(filepath.Join(conceptDir, "concept-manifest.yml"))
		if err != nil {
			a.logger.Debugf("No valid manifest for concept %s: %v", conceptID, err)
			continue
		}
		if !cm.Published(mode) {
			continue
		}

		relFiles, err := concept.MarkdownFiles(conceptDir, cm.DocgenOrder)
		if err != nil {
			a.logger.Warnf("Failed to discover concept docs for %s: %v", conceptID, err)
			continue
		}
		conceptDestDir := filepath.Join(distDest, "concepts", conceptID)
		if err := os.MkdirAll(conceptDestDir, 0o755); err != nil {
		...
		for i, relPath := range relFiles {
			content, err := os.ReadFile(filepath.Join(conceptDir, relPath))
			if err != nil {
				a.logger.Warnf("Failed to read concept doc %s: %v", relPath, err)
				continue
			}
			if transform == "astro" {
				content, err = concept.TransformMarkdown(content, concept.TransformOptions{
			...
			destPath := filepath.Join(conceptDestDir, relPath)
			if err := os.MkdirAll(filepath.Dir(destPath), 0o755); err != nil {
				a.logger.Errorf("Failed to create concept doc directory: %v", err)
				continue
			}
