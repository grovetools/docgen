...
**Important**: First, read the `docgen.config.yml` file to understand the existing configuration and documentation sections that have been defined. Check if there's a `docs/prompts/` directory with section-specific prompts that correspond to the sections in the config file.

Then, I will analyze your project's codebase using the context provided by {{ if .Vars.rules_file }}the rules in `{{ .Vars.rules_file }}`{{ else }}your documentation rules file (typically `docs.rules` or `docs/docs.rules`){{ end }}. 

I will read your document structure from the configuration and also propose additional sections I think might be helpful, such as:
