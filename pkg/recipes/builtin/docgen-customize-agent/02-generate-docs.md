...
1. **Review Configuration and Plan**: 
   - First, read the `docs/docgen.config.yml` file to understand the documentation sections and settings
   - Review the final documentation plan from the chat job to understand the exact requirements
   - Check for any prompts in the `docs/prompts/` directory that correspond to sections defined in the config

2. **Generate Documentation**: For each section defined in the plan and config:
   ...
   - If a section has a corresponding prompt file in `docs/prompts/` (e.g., `docs/prompts/overview.md` for the overview section), use that prompt as the primary guide for generating that section
   - Generate documentation content following the specific prompts agreed upon in the plan and any prompt files
