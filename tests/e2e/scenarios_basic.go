	...
	return &harness.Scenario{
		...
		Steps: []harness.Step{
			...
			harness.NewStep("help command", func(ctx *harness.Context) error {
				...
				if !strings.Contains(result.Stdout, "LLM-powered, workspace-aware documentation generator.") {
					return fmt.Errorf("expected help output to contain 'LLM-powered, workspace-aware documentation generator.'")
				}
				return nil
			}),
		},
